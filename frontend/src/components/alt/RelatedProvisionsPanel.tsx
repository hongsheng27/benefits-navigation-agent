...
export function RelatedProvisionsPanel({
  ...
  const lifeEventLabel = lifeEventId
    ...
    : resolvedLifeEventIds[0]
      ? lifeEventName(resolvedLifeEventIds[0])
      : "目前情況";

  const context: CopilotContext = {
    kind: "related_provisions",
    lifeEventId: lifeEventId ?? resolvedLifeEventIds[0] ?? null,
    lifeEventLabel,
  ...
  return (
    ...
    >
      <p className="rounded-sm border border-[#e2d3b5] bg-[#f8f3ea] px-3 py-2.5 text-[0.82rem] leading-[1.75] text-[#5c564e]">
        這些是候選摘錄，不是已人工核對的全國法規條號。實際申請請以各機關最新公告為準。
      </p>

      {provisions.length === 0 ? (
        <p className="mt-5 rounded-sm border border-dashed border-[#d8cfc0] bg-[#f7f4ee] px-4 py-6 text-[0.92rem] leading-[2] text-[#6b6459]">
          這一輪結果還沒有對應的法條摘錄。請先對照結果清單，或直接洽詢承辦單位；我們不會用其他情境的資料代替。
        </p>
      ) : (
