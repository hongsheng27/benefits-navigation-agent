...
- [x] 1. 建立 schema、migration 與 legacy preservation 基礎
  ...
  - [x] 1.3 建立 typed Entitlement Graph schema
  ...
  - [x] 1.4 建立 canonical Rule DSL、evidence 與 attachment schema
  ...
  - [x] 1.5 建立 coverage、refresh jobs 與 compatibility generations schema
  ...
  - [x] 1.6 實作 legacy `program_rule_fields` preservation 與 candidate conversion migration
  ...
  - [x] 1.7 建立 migration dry-run、rollback 與 committed-state integration tests
...
- [x] 2. 對齊既有 orchestration contracts、errors 與四個 Protocol ports
  - [x] 2.1 對齊既有 immutable shared contracts 與 storage-neutral errors
  ...
  - [x] 2.2 對齊既有 exact storage-neutral Protocol ports
  ...
  - [x] 2.3 擴充既有 required exact unit／contract tests
  ...
  - [x] 2.4 加入 owner-approved pinned Hypothesis dev dependency
  ...
  - [x] 2.5 撰寫 Property 1：immutable contracts 與 amount shape
...
- [x] 3. 建立 SQLite connection／transaction lifecycle helpers
  - [x] 3.1 實作 `contextlib.closing` connection 與 transaction helpers
  ...
  - [x] 3.2 撰寫 lifecycle failure-injection tests
...
- [x] 4. 實作 SQLite adapters 與 storage mapping
  - [x] 4.1 實作 row mapping 與 canonical rule reader
  ...
  - [x] 4.2 實作 Entitlement Graph adapter path semantics
  ...
  - [x] 4.3 實作 Evidence repository exact mapping
  ...
  - [x] 4.4 實作 refresh／coverage SQLite adapter primitives
  ...
  - [x] 4.5 撰寫 required adapter contract／integration tests
  ...
  - [x] 4.6 撰寫 Property 2：Graph path 保留與排除語意
  ...
  - [x] 4.7 撰寫 Property 3：Graph deterministic ordering
...
- [x] 5. 實作純 recursive Rule DSL validation 與 evaluator
  - [x] 5.1 建立 canonical immutable DSL tree 與 validator
  ...
  - [x] 5.2 實作 pure deterministic recursive evaluator
  ...
  - [x] 5.3 實作 required fields、StructuredReason 與 amount mapping
  ...
  - [x] 5.4 撰寫 required DSL／evaluator unit tests
  ...
  - [x] 5.5 撰寫 Property 4：Rule DSL recursive semantics
  ...
  - [x] 5.6 撰寫 Property 5：Missing fields 阻止完整 evaluation
...
- [x] 6. 建立 deterministic、lossless、read-only compatibility projection
  - [x] 6.1 實作 canonical projection converter 與 reverse converter
  ...
  - [x] 6.2 實作 atomic generation persistence 與 read-only view enforcement
  ...
  - [x] 6.3 撰寫 migration comparison 與 cutover tests
  ...
  - [x] 6.4 撰寫 Property 6：Converter deterministic lossless round trip
  ...
  - [x] 6.5 撰寫 Property 7：Projection read-only atomic replacement
...
- [x] 7. 實作 Eligibility service、sorting 與 API／workflow compatibility mapping
  - [x] 7.1 實作 status gates、rule selection 與 citation completeness orchestration
  ...
  - [x] 7.2 實作 deterministic candidate total ordering
  ...
  - [x] 7.3 實作 domain→workflow 與 owner-aware API mappers
  ...
  - [x] 7.4 撰寫 required service／sorting／API compatibility tests
  ...
  - [x] 7.5 撰寫 Property 8：Program status gate matrix
  ...
  - [x] 7.6 撰寫 Property 9：Candidate ordering 與 score non-exposure
  ...
  - [x] 7.7 撰寫 Property 10：Citation exact mapping 與 completeness
...
- [x] 8. 建立隱私 sanitizer、RawTextScope 與 fail-closed observability
  - [x] 8.1 實作 recursive PrivacySanitizer
  ...
  - [x] 8.2 實作 requesting-user authorization mapping 與 RawTextScope disposal
  ...
  - [x] 8.3 將 observability 統一改為 sanitize→validate→serialize→emit
  ...
  - [x] 8.4 撰寫 required privacy lifecycle／authorization tests
  ...
  - [x] 8.5 撰寫 Property 11：Requesting-user response authorization
  ...
  - [x] 8.6 撰寫 Property 12：Recursive sanitizer 與 fail-closed observability
  ...
  - [x] 8.7 撰寫 Property 13：Raw text disposal
...
- [x] 9. 完成 FastAPI composition root、fakes 與 architecture boundaries
  - [x] 9.1 實作唯一 FastAPI composition root
  ...
  - [x] 9.2 建立 no-SQL fakes 並注入 Workflow／state machine
  ...
  - [x] 9.3 撰寫 architecture 與 fake-startup tests
...
- [x] 10. 建立六個 MVP IDs 的安全 migration／curation scaffolding
  - [x] 10.1 建立只含六個既有 ID 的 catalog scaffold
  ...
  - [x] 10.2 實作 human review transition scaffolding
  ...
  - [x] 10.3 撰寫 MVP scaffold 與 synthetic isolation tests
...
- [x] 11. 實作 current-data-first local refresh worker 與 coverage invariants
  - [x] 11.1 實作 request-start snapshot 與 non-blocking local worker boundary
    ...
    - Expected files/modules：`backend/app/orchestration/refresh_orchestration.py`、`backend/app/orchestration/local_worker.py`、`backend/app/orchestration/state_machine.py`。
  ...
  - [x] 11.2 實作 timezone-aware concurrency-safe same-day dedup
  ...
  - [x] 11.3 實作 coverage tracker 與 honest response mapping
  ...
  - [x] 11.4 撰寫 required refresh／coverage integration tests
  ...
  - [x]* 11.5 撰寫 Property 14：Concurrent same-day refresh dedup
  ...
  - [x]* 11.6 撰寫 Property 15：Current-data-first non-blocking refresh
  ...
  - [x]* 11.7 撰寫 Property 16：Coverage snapshot invariants
  ...
  - [x]* 11.8 撰寫 Property 17：Coverage gap 誠實呈現
...
- [x] 12. 建立 later curation pipeline 的 local／mock scaffolding
  - [x] 12.1 實作 registered-source structural discovery
  ...
  - [x] 12.2 實作 local attachment metadata／extraction handling
  ...
  - [x] 12.3 實作 local/mock page classification 與 candidate extraction
  ...
  - [x] 12.4 串接 human review transitions 與 candidate artifacts
  ...
  - [x] 12.5 撰寫 required local/mock curation 與 governance tests
  ...
  - [x]* 12.6 撰寫 Property 20：Pre-August local/mock governance
...
- [x] 13. 選用 tests／release JSON exporter（不得成為 runtime prerequisite）
  - [x] 13.1 實作 deterministic atomic one-way JSON exporter
  ...
  - [x] 13.2 撰寫 exporter atomicity 與 zero-runtime-dependency architecture tests
  ...
  - [x]* 13.3 撰寫 Property 19：JSON deterministic atomic export 與 runtime isolation
...
- [x] 14. 建立 validation CLI、integration suite 與完整 PBT profile
  - [x] 14.1 實作 catalog validation CLI
  ...
  - [x] 14.2 建立 cross-layer integration suite
  ...
  - [x]* 14.3 整合並執行 owner-approved PBT profile
  ...
  - [x]* 14.4 撰寫 Property 18：SQLite lifecycle trace 與 closure
...
- [x] 15. 執行 final implementation validation
  - [x] 15.1 執行完整 safety checkpoint 與 diff validation
...
- Schema migration Tasks 1.1–1.7 已完成；其餘 repositories、runtime cutover 與 feature tasks 仍保持未完成。
