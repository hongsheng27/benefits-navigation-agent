...
> **2026-07-30 第四次更新：事件辨識接上真實模型了。**
> 送什麼文字就真的會被辨識，不再是一律回配偶過世。**所以 `event_not_recognized`
> 這個錯誤現在真的會出現**，前端必須實作它 —— 詳見一之五。
> 目前後端只認得三個事件代號（`spouse_death`、`parent_death`、`child_death`）。
> 契約本身沒有再變，這一次前端要補的是**畫面與文案**，不是型別。
>
> **2026-07-30 第三次更新：契約有一個變更，前端要跟。**
> 新增錯誤代號 **`event_not_recognized`**。`frontend/src/types/session.ts`
...
| 事件辨識（LLM） | ✅ 完成 | 後端有設金鑰時是真的辨識；沒設金鑰時一律回 `spouse_death`。見一之五 |
...
| 只認得三個事件代號 | 🟡 內容是 draft | `spouse_death`、`parent_death`、`child_death`。清單會變長，前端不用改程式 |
| 前端呼叫程式 | ❌ 未實作 | `client.ts` 目前只有健康檢查 |
| 前端 PII 遮罩 | ❌ 未實作 | 目前只有文字提醒 |

**所以現在的實際情況是**：端點可以呼叫、狀態機會真的按規則推進、**事件辨識是真的**
（後端有設金鑰時）、回答欄位會讓項目逐項定案。剩下兩個限制：**判定結果目前只會是
需人工協助**（資料尚未經人工核對，見一之三）、**決定性條件為空**（等資料層配合）。

**還有一件事前端還沒被要求做，但很快會需要**：使用者在第一句話裡順便講的資訊
（例如「我太太走了，我 68 歲」）目前**不會**被抽出來，所以年齡還是會被問一次。
那是後端的下一個任務（T21b），做完之後 `questionGroups` 會變少，前端不用改程式。
...
**這個錯誤現在真的會出現了。** 2026-07-30 事件辨識已經接上真實模型。
...
| 使用者輸入 | 後端回什麼 |
| --- | --- |
| 我先生上個月過世了 | `lifeEvent: "spouse_death"` |
| 我太太走了，我不知道該辦什麼 | `lifeEvent: "spouse_death"` |
| 我媽媽前天在醫院過世 | `lifeEvent: "parent_death"` |
| 我想問問看有什麼補助可以申請 | **422 `event_not_recognized`** |
| 今天天氣真好 | **422 `event_not_recognized`** |

第四個案例值得前端注意：**使用者問了一個合理的問題，但後端拒絕回答。** 這是刻意的 ——
事件代號決定後面七步展開什麼，猜錯會讓使用者被問一整串跟他無關的問題。所以那個畫面的
文案很重要，不能讓使用者覺得自己做錯了什麼。

### 目前後端只認得三個事件

`spouse_death`、`parent_death`、`child_death`。清單在
`data/life_events/events.v0.1.json`，由政策資料負責人維護。

**前端要為這三個代號準備文案**，而且要能處理「清單變長」—— 新增事件只會改那份 JSON，
不會改契約。
