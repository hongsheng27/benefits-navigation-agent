...
契約、端點、狀態機、欄位登記表、缺漏欄位計算、規則引擎轉接、隱私閘門、
**LLM 隔離層與事件辨識**都完成了。使用者打一段話會真的被辨識成事件代號
（已用 `gemma-4-31b-it` 實測），回答欄位會逐項觸發判定，不合法的欄位代號與值都會被拒絕。

剩下三個限制：**判定結果目前只會是「需人工協助」**（示範資料沒有經過人工審查，
依 ADR-0014 不得下結論）、**官方依據檢索是空操作**、**決定性條件為空**（等資料層）。
白話解釋還沒做。前端還沒接上。
...
| T13 | 自由文字只存在第一步 | ✅ | 負責人 | 2026-07-30 在 T21a 完成 |
...
### T13 已在 T21a 完成（2026-07-30）

原本延後的理由是：事件辨識寫死，根本沒讀那段文字，寫丟棄邏輯會沒有東西可丟。

T21a 接上模型之後那段文字真的被使用了，所以這一項現在有實質內容：`resolve_life_event`
收下文字、送給模型、**只回傳一個事件代號**。原文只存在於那個函式的參數裡，呼叫端拿不到
它，所以沒有任何路徑可以把它存到 state、紀錄檔或回應。

有一個測試專門驗證這件事，而且**三條路徑都測**（成功、模型說看不懂、模型壞掉）——
失敗路徑最容易漏，因為處理例外時最順手的寫法就是把輸入放進訊息。

`implementation.pending` 因此移除 `privacy_gate`，剩下六項。
...
### T19、T20、T21a 做了什麼（2026-07-30）

`app/llm/port.py`（形狀與契約）、`app/llm/fake.py`（離線實作）、
`app/llm/tasks/resolve_life_event.py`（事件辨識）、
`app/orchestration/life_events.py` 與 `data/life_events/events.v0.1.json`（事件登記表）。

**事件代號的清單放在 JSON 登記表，不在程式裡。** `state.py` 有一條原則說事件集合屬於
entitlement graph 的 curated 資料，寫死在程式裡等於把政策放進應用程式碼。嚴格來說應該
在 `EntitlementGraphRepository` 加一個 `list_events()`，但 `protocols.py` 正被
`feat/databaseV3` 大幅修改，現在動它會製造衝突。**這是暫行安排**，資料層之後若在 graph
契約上提供事件清單，這份 JSON 應該退場。

**schema 用 `enum` 把模型限制在登記表內。** 這比隱私閘門更強：模型不是被攔下來，
是根本沒有別的選項。另外給一個 `unrecognised` 值，讓「判斷不出來」是一個誠實的答案
而不是逼它硬選。

**T21 拆成 a 與 b。** 屬性抽取（T21b）需要**從欄位登記表動態生成 schema**，因為 Bedrock
要求 `additionalProperties: false`，所以不能有「任意鍵值對」的欄位，每個允許的屬性都要
...
`life_event` 留成 `None`，使用者會看到同一個輸入畫面而沒有任何說明 —— 對一個正在辦
喪事的人來說那很糟。

所以加了 `event_not_recognized`（HTTP 422），前端據此顯示「我們沒有看懂，可以換個說法
嗎」。這是**對外契約變更**，後端列舉、前端型別、走鐘檢查測試三處都改了。

**刻意不區分「模型壞掉」與「描述看不懂」** —— 對使用者而言下一步都一樣，區分只對我們
有意義，記在紀錄檔就好。

**刻意不做「從清單挑一個事件」。** 那需要在快照加一個候選事件清單欄位，還要加一種新的
輸入類型，是更大的契約決定，要跟前端一起討論。

### 一個為了讓示範能動而做的取捨

`advance()` 的預設語言模型沒有登記任何答案，所以**預設情況下事件辨識會失敗**。那是誠實
的預設值：沒有真模型時系統確實看不懂。

但端點層注入了 `demo_language_model()`（一律回配偶過世），否則產品在第一步就停住，
什麼都示範不了。這跟 ADR-0014 讓判定維持「需人工協助」的做法不一致，理由是**兩者後果
不對等**：「需人工協助」是使用者可以據此行動的結局，事件辨識失敗則是什麼都做不了。

這件事不是默默發生的 —— 回應的 `implementation.pending` 一直帶著
`life_event_extraction`。T23 接上 Gemini 後，有金鑰時改用真實 adapter，沒有金鑰才落回
示範模型。
...
| 07-30 | T21 | 拆成 T21a（事件辨識）與 T21b（屬性抽取） | 屬性抽取需要從欄位登記表動態生成 schema，因為 Bedrock 不允許「任意鍵值對」的欄位 |
